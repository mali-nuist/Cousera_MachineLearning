...
%% and x_0 to the first column
X = [ones(m,1), X];
a1 = X;
z2 = a1 * Theta1'; 
a2 = sigmoid(z2);
a2 = [ones(size(a2,1),1),a2]; % add bias units of a2
z3 = a2 * Theta2';
a3 = sigmoid(z3);
hx = a3;
yt = eye(num_labels)(y,:);
J = -1/m * sum(sum(log(hx).*yt + (1-yt).*log(1-hx),2))+lambda/(2*m)*(sum(sum(Theta1(:,2:end).^2))+sum(sum(Theta2(:,2:end).^2)));
...
for i = 1 : m,
	...
	delta3 = (a3(i,:) - yt(i,:))';
	delta2 = Theta2(:,2:end)' * delta3 .* gz2(i,:)';
